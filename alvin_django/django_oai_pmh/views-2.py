...
def oai2(request):
    ...
    if "verb" in params:
        ...
        if verb == "GetRecord":
                ...
                if len(metadata_prefix) == 1:
                    ...
                    if "identifier" in params:
                        ...
                        if response.status_code == 200:
                            ...
                            records = []
                            for record in xml_record.findall('./data/record'):
                                records.append({
            ...
            list_url = f'https://cora.alvin-portal.org/rest/record/searchResult/alvinRecordSearch?searchData={{"name":"alvinRecordSearch","children":[{{"name":"include","children":[{{"name":"includePart","children":[{{"name":"permissionUnitSearchTerm","value":"permissionUnit_{set}"}}]}}]}}]}}'
            ...
            if not sets:
                ...
                (
                    ...
                    from_timestamp,
                    until_timestamp,
                ) = _do_resumption_token(params, errors, sets)
            _check_bad_arguments(params, errors)
        else:
