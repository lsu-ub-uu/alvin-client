...
def generate_resumption_token(start, page_size, total_records, metadataPrefix, set):
    state = f"start={start}:{page_size}:{total_records}:metadataPrefix={metadataPrefix}:set={set}"
...
def xml_feed(request):
    ...
    if cursor + page_size < total_records:
        token_value = generate_resumption_token(start, page_size, total_records, metadataPrefix, set)
    else:
        token_value = ""


    return render(request, 'oai_rt.xml', {'state':state, 'start':start,'token_value':token_value, 'cursor':cursor, 'page_size':page_size, 'total_records':total_records, 'set':set, 'metadataPrefix': metadataPrefix}, content_type='text/xml')
