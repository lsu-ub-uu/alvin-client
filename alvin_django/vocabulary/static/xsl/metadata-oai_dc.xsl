        ...
        <record>
            ...
            http://www.openarchives.org/OAI/2.0/oai_dc.xsd">
                    ...
                    <xsl:for-each select="physicalLocation">
                        <dc:identifier>
                            <xsl:value-of select="heldBy/location/linkedRecord/location/authority[1]/name/namePart[@type = 'corporateName']"/>
                            <xsl:if test="string-length(subcollection) &gt; 0">
                                <xsl:for-each select="subcollection">
                                    <xsl:text>, </xsl:text>
                                    <xsl:for-each select="uu">
                                        <xsl:value-of select="@_value_en"/>
                                    </xsl:for-each>
                                </xsl:for-each>
                            </xsl:if>
